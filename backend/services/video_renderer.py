...
from moviepy.editor import (AudioFileClip, ColorClip, CompositeVideoClip,
                            ImageClip, TextClip, concatenate_videoclips, CompositeAudioClip, afx, VideoFileClip, vfx, audio_normalize, concatenate_audioclips)
...
class VideoRendererService:
    ...
    def tts_client(self) -> Optional[texttospeech.TextToSpeechClient]:
        ...
        if self._tts_client is None:
            try:
                ...
                logger.info("Cliente Google Cloud Text-to-Speech inicializado com sucesso.")
            except Exception as e:
                logger.error(f"Falha ao inicializar o cliente Text-to-Speech: {e}", exc_info=True)
    ...
    async def render_video(
        ...
        if aspect_ratio not in resolutions or resolution not in resolutions[aspect_ratio]:
            raise ValueError("Proporção ou resolução inválida.")
        
        target_size = resolutions[aspect_ratio][resolution]
        logger.info(f"Renderizando vídeo em {resolution} com proporção {aspect_ratio} ({target_size[0]}x{target_size[1]})")
        ...
        if not os.path.exists(cached_narration_path):
            await self._generate_narration(full_script_text, cached_narration_path, voice_name)
        else:
        ...
        if background_music_path:
            background_music = AudioFileClip(background_music_path).subclip(0, total_duration)

            # Reduz o volume da música de fundo para não sobrepor a narração
            background_music = background_music.volumex(0.3)  # Ajuste este valor conforme necessário
        ...
        video_scenes = assets.get("scenes", {}) # Alterado de "images" para "scenes" para ser mais genérico
        ...
        for i, (scene_key, scene_data) in enumerate(video_scenes.items()):
            ...
            scene_text = structured_script[i]["text"] if i < len(structured_script) else ""
            clip = None
            ...
            if asset_path.lower().endswith(('.mp4', '.mov', '.avi')):
                ...
                def resize_func(t):
                    if zoom_direction == "in":
                        ...
                        return zoom_amount - (zoom_amount - 1) * (t / duration_per_scene)

                clip = img_clip.resize(resize_func)

            # Garante que todos os clipes tenham o tamanho e a proporção corretos
            clip = clip.resize(width=target_size[0] if aspect_ratio != '9:16' else None, height=target_size[1] if aspect_ratio == '9:16' else None)
        ...
        if len(final_clips) > 1:
            ...
            main_content_video = concatenate_videoclips(clips_with_transition, padding=-transition_duration, method="compose")
        elif final_clips:
        ...
        if watermark_path:
            ...
            watermark_clip = (ImageClip(watermark_path)
                              ...
                              .set_pos(("right","top"))
                              .set_opacity(0.75)) # Ajusta a opacidade
        ...
        youtube_chapters = self._generate_youtube_chapters(script_data, final_video.duration)
    ...
    def _generate_youtube_chapters(self, script_data: Dict[str, Any], total_duration: float) -> list[str]:
        """
